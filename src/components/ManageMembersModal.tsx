...
export function ManageMembersModal({ topic, onClose }: ManageMembersModalProps) {
      ...
      <div className="bg-white rounded-2xl shadow-xl w-80 max-h-[85vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
          <span className="font-semibold text-gray-800 text-sm">
            🔒 {topic.name} — {t('topic.manageMembers')}
          </span>
        ...
        <div className="overflow-y-auto flex-1 flex flex-col">
          ...
          <div className="divide-y divide-gray-50">
            {allUsers.map((u) => {
              const isOwner = u.uid === user?.uid;
              const checked = selectedUIDs.has(u.uid);
              return (
                <label
                  key={u.uid}
                  className={`flex items-center gap-3 px-4 py-2.5 text-sm ${
                    isOwner ? 'bg-gray-50 cursor-default' : 'hover:bg-gray-50 cursor-pointer'
                  }`}
                >
                  <input
                    type="checkbox"
